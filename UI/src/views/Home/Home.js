...
class Home extends React.Component {
    ...
    async endLoading() {
        ...
        const profile = await ApiService.apiRequest(ApiService.ROUTES.GET.GET_PROFILE)
        this.openProfile(profile)
    }

    getProfiles() {
        const mock = {
            name: 'Tomer Cohfsfden',
            job: 'kayhut',
            blabla: 'blabla'
        ...
        return mocks.map((mock, index) => (
            <div key={`image-${index}`} className='image-wrapper' onClick={() => this.openProfile(mock)}>
                <img src={Trump} alt=''/>
                <span className='img__description'> {mock.name}</span>
            </div>
        ))
    }
